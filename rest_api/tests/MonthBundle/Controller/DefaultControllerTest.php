...
class DefaultControllerTest extends WebTestCase
{
    ...
    public function testUpdateDays(){
      ...
      $client = $this->sendJson("/updateMonth", $json, 'PUT');
      $this->assertTrue(
        $client->getResponse()->headers->contains('Content-Type', 'application/json'),
        'the "Content-Type" header is "application/json"' // optional message shown on failure
      );
    ...
    public function testCreateMonth(){
      $json = $this->makeMonth();
      $client = $this->sendJson("/createMonth", $json, 'POST');
      $this->assertTrue(
        $client->getResponse()->headers->contains('Content-Type', 'application/json'),
        'the "Content-Type" header is "application/json"' // optional message shown on failure
      );
    ...
    public function testSaveValidDay(){
      $json = $this->dayTemplate("01");
      $client = $this->sendJson("/saveDay", $json, 'PUT');
      $this->assertTrue(
        $client->getResponse()->headers->contains('Content-Type', 'application/json'),
        'the "Content-Type" header is "application/json"' // optional message shown on failure
      );
      $content = $client->getResponse()->getContent();
      $this->assertGreaterThan(5, strlen($content));
      $this->assertContains("saved", $content);
    }

  public function testSaveDayInvalidDate(){
      $json = '{"id":"null","day":"","single":{"selected":true,"price":999,"available":999,"type":"single"},"double":{"selected":false,"price":200,"available":3,"type":"double"}}';
      $client = $this->sendJson("/saveDay", $json, 'PUT');
      $this->assertTrue(
        $client->getResponse()->headers->contains('Content-Type', 'application/json'),
        'the "Content-Type" header is "application/json"' // optional message shown on failure
      ...
      $content = $client->getResponse()->getContent();
      $this->assertGreaterThan(5, strlen($content));
      $this->assertContains("no_date", $content);
  }

    public function testSaveDayInvalidAvailable(){
      $json = '{"id":"null","day":"2000-01-01T00:00:00.000Z","single":{"selected":true,"price":"","available":"","type":"single"},"double":{"selected":false,"price":"","available":"","type":"double"}}';
      $client = $this->sendJson("/saveDay", $json, 'PUT');
      $this->assertTrue(
        $client->getResponse()->headers->contains('Content-Type', 'application/json'),
        'the "Content-Type" header is "application/json"' // optional message shown on failure
      ...
      $content = $client->getResponse()->getContent();
      $this->assertGreaterThan(5, strlen($content));
      $this->assertContains("blank", $content);
  }
